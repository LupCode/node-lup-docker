...
export type DockerCreateContainerOptions = {
    ...
    /** Wether the command is already escaped (Windows only, default false). */
    argsEscaped?: boolean;
    ...
    /** Wether networking for the container should be disabled. */
...
export type DockerGetContainerStatsOptions = {
    ...
    /** Wether to keep the connection open and stream stats continuously (default false). */
    stream?: boolean;
};
...
export type DockerImageMetadata = {
    ...
    /** Content data if already present in binary format. */
    data?: Buffer;
