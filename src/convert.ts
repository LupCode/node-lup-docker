import {
  DockerContainer,
  DockerContainerBlockIoStatEntry,
  DockerContainerCpuStats,
  DockerContainerHealthState,
  DockerContainerMemoryStats,
  DockerContainerMountInfo,
  DockerContainerNetwork,
  DockerContainerNetworkMode,
  DockerContainerNetworkStats,
...
export function encodeCreateContainerOptions(image: string, options?: DockerCreateContainerOptions): string {
  ...
  if (options?.env) {
    body.Env = [];
    for (const [key, value] of Object.entries(options.env)) body.Env.push(key + '=' + (value ?? ''));
  }
  ...
  if (options?.healthCheck) {
    ...
    body.Healthcheck = {
      Interval:
        (options.healthCheck.interval || 0) <= 0 ? 0 : Math.max(1000000, options.healthCheck.interval! * 1000000000), // sec to ns
      Retries: Math.max(0, options.healthCheck.retries || 0),
      StartInterval:
        (options.healthCheck.startInterval || 0) <= 0
          ? 0
          : Math.max(1000000, options.healthCheck.startInterval! * 1000000000), // sec to ns
      StartPeriod:
        (options.healthCheck.startPeriod || 0) <= 0
          ? 0
          : Math.max(1000000, options.healthCheck.startPeriod! * 1000000000), // sec to ns
      Test:
        options.healthCheck.test.type === 'arguments'
          ? ['CMD', ...(options.healthCheck.test.arguments || [])]
          : options.healthCheck.test.type === 'command'
            ? ['CMD-SHELL', options.healthCheck.test.command || '']
            : options.healthCheck.test.type === 'inherit'
              ? []
              : options.healthCheck.test.type === 'none'
                ? ['NONE']
                : [options.healthCheck.test.type],
      Timeout:
        (options.healthCheck.timeout || 0) <= 0 ? 0 : Math.max(1000000, options.healthCheck.timeout! * 1000000000), // sec to ns
    };
  }
  if (options?.hostConfig) {
    ...
    if (options.hostConfig.binds)
      body.HostConfig.Binds = options.hostConfig.binds.map<string>((bind) => {
        let str = bind.hostPathOrVolumeName + ':' + bind.containerPath;
        if (bind.options) {
          const bindOptions: string[] = [];
          if (bind.options.noCopy) bindOptions.push('nocopy');
          if (bind.options.propagation) bindOptions.push(bind.options.propagation);
          if (bind.options.readOnly) bindOptions.push('ro');
          else bindOptions.push('rw');
          if (bind.options.shared !== undefined) bindOptions.push(bind.options.shared ? 'z' : 'Z');
          if (bindOptions.length > 0) str += ':' + bindOptions.join(',');
        }
        return str;
      });
    if (options.hostConfig.blockIoDeviceReadBps)
      body.HostConfig.BlkioDeviceReadBps = Object.entries(options.hostConfig.blockIoDeviceReadBps).map((entry) => {
        if (typeof entry[1] === 'object') return { Path: entry[0], ...(entry[1] as object) };
        return { Path: entry[0], Rate: entry[1] };
      });
    if (options.hostConfig.blockIoDeviceReadOps)
      body.HostConfig.BlkioDeviceReadIOps = Object.entries(options.hostConfig.blockIoDeviceReadOps).map((entry) => {
        if (typeof entry[1] === 'object') return { Path: entry[0], ...(entry[1] as object) };
        return { Path: entry[0], Rate: entry[1] };
      });
    if (options.hostConfig.blockIoDeviceWeight)
      body.HostConfig.BlkioWeightDevice = Object.entries(options.hostConfig.blockIoDeviceWeight).map((entry) => {
        if (typeof entry[1] === 'object') return { Path: entry[0], ...(entry[1] as object) };
        return { Path: entry[0], Weight: entry[1] };
      });
    if (options.hostConfig.blockIoDeviceWriteBps)
      body.HostConfig.BlkioDeviceWriteBps = Object.entries(options.hostConfig.blockIoDeviceWriteBps).map((entry) => {
        if (typeof entry[1] === 'object') return { Path: entry[0], ...(entry[1] as object) };
        return { Path: entry[0], Rate: entry[1] };
      });
    if (options.hostConfig.blockIoDeviceWriteOps)
      body.HostConfig.BlkioDeviceWriteIOps = Object.entries(options.hostConfig.blockIoDeviceWriteOps).map((entry) => {
        if (typeof entry[1] === 'object') return { Path: entry[0], ...(entry[1] as object) };
        return { Path: entry[0], Rate: entry[1] };
      });
    ...
    if (options.hostConfig.consoleSize)
      body.HostConfig.ConsoleSize = [
    ...
    if (options.hostConfig.cpuRealtimePeriod !== undefined)
      body.HostConfig.CpuRealtimePeriod = options.hostConfig.cpuRealtimePeriod;
    if (options.hostConfig.cpuRealtimeRuntime !== undefined)
      body.HostConfig.CpuRealtimeRuntime = options.hostConfig.cpuRealtimeRuntime;
    if (options.hostConfig.cpuSetCpus)
      body.HostConfig.CpuSetCpus = options.hostConfig.cpuSetCpus.map<string>((cpu) => cpu.toString()).join(',');
    if (options.hostConfig.cpuSetMemoryNodes)
      body.HostConfig.CpuSetMems = options.hostConfig.cpuSetMemoryNodes
    ...
    if (options.hostConfig.deviceRequests)
      body.HostConfig.DeviceRequests = options.hostConfig.deviceRequests.map((req) => ({
    ...
    if (options.hostConfig.devices)
      body.HostConfig.Devices = options.hostConfig.devices.map((device) => ({
    ...
    if (options.hostConfig.extraHosts)
      body.HostConfig.ExtraHosts = Object.entries(options.hostConfig.extraHosts).map<string>(
    ...
    if (options.hostConfig.ipcMode) {
      ...
      if (options.hostConfig.ipcMode.mode === 'container') {
        if (!options.hostConfig.ipcMode.containerIdOrName)
          throw new Error('Container ID or name must be specified for IPC mode "container"');
        body.HostConfig.IpcMode = 'container:' + options.hostConfig.ipcMode.containerIdOrName;
      }
    }
    if (options.hostConfig.kernelMemoryTCP !== undefined)
      body.HostConfig.KernelMemoryTCP = options.hostConfig.kernelMemoryTCP;
    if (options.hostConfig.links)
      body.HostConfig.Links = options.hostConfig.links.map((link) => link.containerName + ':' + link.alias);
    if (options.hostConfig.logging)
    ...
    if (options.hostConfig.memoryReservation !== undefined)
      body.HostConfig.MemoryReservation = options.hostConfig.memoryReservation;
    if (options.hostConfig.memorySwap !== undefined) body.HostConfig.MemorySwap = options.hostConfig.memorySwap;
    if (options.hostConfig.memorySwappiness !== undefined)
      body.HostConfig.MemorySwappiness = options.hostConfig.memorySwappiness;
    if (options.hostConfig.mounts)
      body.HostConfig.Mounts = options.hostConfig.mounts.map((mount) => ({
        ...
        TmpfsOptions: mount.tmpfsOptions
          ? {
              ...
              Options: Object.entries(mount.tmpfsOptions.options || {}).map<string[]>((entry) =>
                entry[1] ? [entry[0], entry[1]] : [entry[0]],
              ),
            }
          : undefined,
        Type: mount.type,
    ...
    if (options.hostConfig.networkMode)
      body.HostConfig.NetworkMode = encodeDockerContainerNetworkMode(options.hostConfig.networkMode);
    if (options.hostConfig.outOfMemoryKillDisable !== undefined)
      body.HostConfig.OomKillDisable = options.hostConfig.outOfMemoryKillDisable;
    if (options.hostConfig.outOfMemoryScore !== undefined)
      body.HostConfig.OomScoreAdj = options.hostConfig.outOfMemoryScore;
    ...
    if (options.hostConfig.publishAllPorts !== undefined)
      body.HostConfig.PublishAllPorts = options.hostConfig.publishAllPorts;
    if (options.hostConfig.readOnlyPaths) body.HostConfig.ReadonlyPaths = options.hostConfig.readOnlyPaths;
    if (options.hostConfig.restart)
    ...
    if (options.hostConfig.rootFsReadOnly !== undefined)
      body.HostConfig.ReadonlyRootfs = options.hostConfig.rootFsReadOnly;
    ...
    if (options.hostConfig.tmpfs) {
      ...
      for (const tmpfs of options.hostConfig.tmpfs) {
        const args: string[] = [];
        if (tmpfs.deviceFile === 'create-only') args.push('nodev');
        else if (tmpfs.deviceFile === 'yes') args.push('dev');
    ...
    if (options.hostConfig.uLimits)
      body.HostConfig.Ulimits = options.hostConfig.uLimits.map((ulimit) => ({
        Hard: ulimit.hard,
        Name: ulimit.name,
        Soft: ulimit.soft,
      }));
    ...
    if (options.hostConfig.volumesInheritFrom)
      body.HostConfig.VolumesFrom = options.hostConfig.volumesInheritFrom.map<string>((volume) => {
  ...
  if (options?.volumes) {
    ...
    for (const volume of options.volumes) volumesObj[volume] = {};
...
export function decodeDockerContainerBlockIoStatEntries(
  json: any[] | undefined,
): DockerContainerBlockIoStatEntry[] | undefined {
  if (!json) return undefined;
    ...
    blockIoStats: json.blkio_stats
      ? {
          ioMergedRecursive: json.io_merged_recursive
            ? decodeDockerContainerBlockIoStatEntries(json.io_merged_recursive)
            : undefined,
          ioQueueRecursive: json.io_queue_recursive
            ? decodeDockerContainerBlockIoStatEntries(json.io_queue_recursive)
            : undefined,
          ioServiceBytesRecursive: json.io_service_bytes_recursive
            ? decodeDockerContainerBlockIoStatEntries(json.io_service_bytes_recursive)
            : undefined,
          ioServicedRecursive: json.io_serviced_recursive
            ? decodeDockerContainerBlockIoStatEntries(json.io_serviced_recursive)
            : undefined,
          ioServiceTimeRecursive: json.io_service_time_recursive
            ? decodeDockerContainerBlockIoStatEntries(json.io_service_time_recursive)
            : undefined,
          ioTimeRecursive: json.io_time_recursive
            ? decodeDockerContainerBlockIoStatEntries(json.io_time_recursive)
            : undefined,
          ioWaitTimeRecursive: json.io_wait_time_recursive
            ? decodeDockerContainerBlockIoStatEntries(json.io_wait_time_recursive)
            : undefined,
          sectorsRecursive: json.sectors_recursive
            ? decodeDockerContainerBlockIoStatEntries(json.sectors_recursive)
            : undefined,
        }
      : undefined,
    containerId: json.id || '',
...
export function encodeDockerCredentials(auth: DockerCredentials | DockerIdentityToken): string {
  if (!auth || typeof auth === 'string') return auth;
  return JSON.stringify(auth);
}

export function decodeDockerImage(json: any | undefined): DockerImage | undefined {
  if (!json) return undefined;
  return {
    containers: json.Containers !== undefined && json.Containers >= 0 ? json.Containers : undefined,
    created: new Date(json.Created * 1000),
    descriptor: decodeDockerOCIDescriptor(json.Descriptor)!,
    digests: json.RepoDigests || undefined,
    imageId: json.Id || '',
    labels: json.Labels || {},
    manifests: (json.Manifests || [])
      .map(decodeDockerImageManifestSummary)
      .filter((m: DockerImageManifestSummary | undefined) => m) as DockerImageManifestSummary[],
    parentId: json.Parent || undefined,
    sharedSize: json.SharedSize !== undefined && json.SharedSize >= 0 ? json.SharedSize : undefined,
    size: json.Size || -1,
    tags: json.RepoTags || [],
  };
}

export function decodeDockerImageManifestSummary(json: any | undefined): DockerImageManifestSummary | undefined {
  if (!json) return undefined;
  return {
    attestationData: json.AttestationData
      ? {
          for: json.AttestationData.For || '',
        }
      : undefined,
    ...
    imageData: json.ImageData
      ? {
          containers: json.ImageData.Containers || [],
          platform: decodeDockerOCIPlatform(json.ImageData.Platform),
          unpackedSize: json.ImageData.Size?.Unpacked || -1,
        }
      : undefined,
    imageId: json.ID || '',
    kind: json.Kind || 'unknown',
    size: {
      content: json.Size?.Content || -1,
      total: json.Size?.Total || -1,
    },
  };
}

export function decodeDockerOCIDescriptor(json: any | undefined): DockerOCIDescriptor | undefined {
  if (!json) return undefined;
  return {
    annotations: json.annotations || {},
    artifactType: json.artifactType ?? undefined,
    data: json.data ? Buffer.from(json.data, 'base64') : undefined,
    digest: json.digest || '',
    mediaType: json.mediaType || '',
    platform: decodeDockerOCIPlatform(json.platform),
    size: json.size || 0,
    urls: json.urls || [],
  };
}

export function encodeDockerOCIPlatform(platform: DockerOCIPlatform | undefined): any | undefined {
  if (!platform) return undefined;
  return {
    architecture: platform.architecture,
    os: platform.os,
    'os.features': platform.osFeatures,
    'os.version': platform.osVersion,
    variant: platform.variant,
  };
}
export function encodeDockerOCIPlatformToJson(platform: DockerOCIPlatform | undefined): any | undefined {
  const json = encodeDockerOCIPlatform(platform);
  return json ? JSON.stringify(json) : undefined;
}
export function decodeDockerOCIPlatform(json: any | undefined): DockerOCIPlatform | undefined {
  if (!json) return undefined;
  return {
    architecture: json.architecture || '',
    os: json.os || '',
    osFeatures: json['os.features'] || [],
    osVersion: json['os.version'],
